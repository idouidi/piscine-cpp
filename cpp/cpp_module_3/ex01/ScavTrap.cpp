...
/*   Updated: 2022/02/02 00:04:53 by user42           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ScavTrap.hpp"

ScavTrap::ScavTrap():ClapTrap()// hitPoint(100), energyPoint(50), attackDamage(20), name("Bot")
{
    hitPoint = 100;
    energyPoint = 50;
    attackDamage = 20;
...
void ScavTrap::attack(const ScavTrap &other)
{
	if (energyPoint)
	{
		energyPoint--;
        if (!other._guard)
		    std::cout << "ClapTrap " << name << " attacks " << other.name << ", causing " << attackDamage << " point of damage!" << std::endl;
        else
